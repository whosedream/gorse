...
func DefaultPromptBuilder() PromptBuilder {
	return PromptBuilder{
		Template: PromptTemplate{
			SystemDirective:   "You are an ecommerce intent extraction node. Return final JSON only. intent_vector is generated by the embedding service — do NOT include it.",
			ReasoningProtocol: "Use behavior chronology to infer category weights. Priority: recency > frequency > dwell time. Output only the JSON structure defined in output_schema. Do not expose hidden reasoning.",
			OutputSchema:      `{"session_id":"string","baseline_version":0,"category_weights":{"category_a":0.0,"category_b":0.0}}`,
		},
		MaxEvents: 64,
	}
}
